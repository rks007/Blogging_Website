...
export function TextEditor({onChange, value} : {onChange: (e: ChangeEvent<HTMLTextAreaElement>) => void, value: string}) {
    ...
    return <form className="flex justify-center"> 
        <div className="max-w-screen-lg w-full mx-20">
          ...
          <textarea value={value} onChange={onChange} id="message" rows={4} className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 " placeholder="Write your Content here..." required></textarea>
        </div>
    </form>
}